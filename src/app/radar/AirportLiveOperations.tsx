...
export default function AirportLiveOperations() {
    ...
    useEffect(() => {
        const onKeyDown = (event: KeyboardEvent) => {
            if (event.key === "Escape") setAirport(null);
        };
    ...
    const operations = useMemo(() => {
        ...
        for (const item of aircraft) {
            ...
            const distance = distanceKm(item.latitude, item.longitude, airport.latitude, airport.longitude);
            if (distance > 120) continue;

            const bearing = bearingDegrees(item.latitude, item.longitude, airport.latitude, airport.longitude);
            ...
            const operation: Operation = { ...item, distanceKm: distance, bearingToAirport: bearing, relativeHeading: relative };
            const arrivalLike = distance <= 100 && relative <= 65 && (verticalFpm < -150 || (altitudeFt != null && altitudeFt < 12000));
            const departureLike = distance <= 90 && relative >= 115 && (verticalFpm > 150 || (altitudeFt != null && altitudeFt < 15000));
        ...
        return { arrivals: arrivals.slice(0, 12), departures: departures.slice(0, 12) };
    }, [airport, aircraft]);
            ...
            <section style={panelStyle}>
                <div style={headerStyle}>
                    ...
                    <button
                        ...
                        title="Schließen (Esc)"
                        aria-label="Airport Operations schließen"
                        style={closeStyle}
                    >
                        ×
                    </button>
...
function OperationSection({ title, items, arrival = false }: { title: string; items: Operation[]; arrival?: boolean }) {
    return (
        <div>
            <div style={sectionHeaderStyle}><span>{arrival ? "↘" : "↗"} {title}</span><strong>{items.length}</strong></div>
            {items.length === 0 ? <div style={emptyStyle}>No matching live aircraft in the current radar snapshot.</div> : (
                <div style={{ display: "grid", gap: 5 }}>
                    {items.map((item) => <OperationRow key={`${title}-${item.icao24}`} item={item} arrival={arrival} />)}
                </div>
            )}
        </div>
    );
...
function distanceKm(lat1: number, lon1: number, lat2: number, lon2: number) { const R = 6371; const toRad = (value: number) => value * Math.PI / 180; const dLat = toRad(lat2 - lat1); const dLon = toRad(lon2 - lon1); const a = Math.sin(dLat / 2) ** 2 + Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLon / 2) ** 2; return 2 * R * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)); }
function bearingDegrees(lat1: number, lon1: number, lat2: number, lon2: number) { const toRad = (value: number) => value * Math.PI / 180; const y = Math.sin(toRad(lon2 - lon1)) * Math.cos(toRad(lat2)); const x = Math.cos(toRad(lat1)) * Math.sin(toRad(lat2)) - Math.sin(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.cos(toRad(lon2 - lon1)); return (Math.atan2(y, x) * 180 / Math.PI + 360) % 360; }
function headingDifference(a: number, b: number) { return Math.abs(((a - b + 540) % 360) - 180); }
...
const headerStyle: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "13px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)" };
